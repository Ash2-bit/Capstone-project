...
// Color map based on overall_severity of each individual report
const severityColors = {
  berat: '#ef4444',   // merah
  sedang: '#eab308',  // kuning
  ringan: '#10b981',  // hijau
  unknown: '#94a3b8', // silver
};

// Color map for cluster circles based on priority_level
const clusterColors = {
  red: '#ef4444',
  yellow: '#eab308',
  green: '#10b981',
...
const createCustomIcon = (color, isSar = false) => {
  const html = isSar
    ...
    : `<div style="background-color: ${color}; border: 2px solid white; border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
...
export default function InteractiveMap({ clusters = [], sarBases = [] }) {
  ...
  if (!isMounted) {
    return (
      <div className="w-full h-full min-h-[500px] bg-slate-800 rounded-xl flex items-center justify-center border border-slate-700 animate-pulse">
                        ...
                        <div className="p-2 w-64 max-h-[350px] overflow-y-auto">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                              report.overall_severity === 'berat' ? 'bg-red-900 text-red-200' :
                              report.overall_severity === 'sedang' ? 'bg-yellow-900 text-yellow-200' :
                              report.overall_severity === 'ringan' ? 'bg-green-900 text-green-200' :
                              'bg-slate-700 text-slate-300'
                            }`}>
                              {report.overall_severity === 'berat' ? '🔴 Berat' :
                               report.overall_severity === 'sedang' ? '🟡 Sedang' :
                               report.overall_severity === 'ringan' ? '🟢 Ringan' :
                               '⚪ Unknown'}
                            </span>
